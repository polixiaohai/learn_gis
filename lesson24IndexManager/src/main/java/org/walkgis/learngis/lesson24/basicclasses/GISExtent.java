...
import org.walkgis.learngis.lesson24.basicclasses.enums.GISMapAction;
...
public class GISExtent {
    ...
    public GISExtent(DataInputStream br) throws IOException {
        this.upRight = new GISVertex(br.readDouble(), br.readDouble());
        this.bottomLeft = new GISVertex(br.readDouble(), br.readDouble());
        this.area = getWidth() * getHeight();
    }
    ...
    public void write(DataOutputStream bw) throws IOException {
        bw.writeDouble(getMaxX());
        bw.writeDouble(getMaxY());
        bw.writeDouble(getMinX());
        bw.writeDouble(getMinY());
    }
}
